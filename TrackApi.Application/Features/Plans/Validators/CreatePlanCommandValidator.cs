...
namespace TrackApi.Application.Features.Plans.Validators
{
    public class CreatePlanCommandValidator : AbstractValidator<CreatePlanCommand>
    {
        ...
        {
            RuleFor(x => x.InputCreationPlanDto.PlanType).NotEmpty().WithMessage(ValidatorMessage.Required);
            RuleFor(x => x.InputCreationPlanDto.StartDate).NotEmpty().LessThanOrEqualTo(x => x.InputCreationPlanDto.EndDate).WithMessage(ValidatorMessage.Required);
            RuleFor(x => x).Must(x => IsValidEndDate(x.InputCreationPlanDto.PlanType, x.InputCreationPlanDto.StartDate, x.InputCreationPlanDto.EndDate)).WithMessage(ValidatorMessage.InvalidDate);
        }
